<section id="strategies" class="w-full py-40 bg-[#181818] text-white">
	<div class="w-[90%] mx-auto text-right max-w-7xl">
		<h2 class="text-4xl text-[#F9B333] font-bold mb-6 pt-10">
			Notre stratégie, étape par étape
		</h2>
		<div class="mx-auto overflow-hidden ">
			<div class="md:flex gap-12 items-stretch">
				<div class="md:shrink-0 md:max-w-md md:flex-1">
					<img
						class="h-48 w-full  rounded object-cover md:h-auto md:max-w-md"
						src="concept/intro.png"
						alt="Screenshot of a trading app "
					/>
				</div>
				<div class="mt-10 md:mt-0 flex-1 flex items-center justify-center">
						<lu id="paraph" class="ml-10 flex-1 md:text-xl space-y-6">
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Stratégie élaborée sur des probabilités et des statistiques pour optimiser le trading de notre trader
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Trading sur le Bitcoin en levier 50
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Stratégie qui s’adapte à tout type de marché : volatile ou peu volatile
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Maximisation des intérêts composés et minimisation des pertes grâce à l’élaboration de paliers de capital
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Risk Management agressif
							</li>
						</lu>
				</div>
			</div>
		</div>
  </div>
</section>

<section id="api-key" class="w-full py-10 bg-[#181818] text-white">
	<div class="w-[90%] mx-auto text-left max-w-7xl">
		<h2 class="text-4xl text-[#F9B333] font-bold mb-6 pt-10">
			La clé API,<br> de A à Z
		</h2>
		<div class="mx-auto overflow-hidden ">
			<div class="md:flex gap-12 items-stretch">
				<div class="mt-10 md:mt-0 flex-1 flex items-center justify-center">
						<lu id="paraph" class="ml-10 flex-1 md:text-xl space-y-6">
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Votre argent reste sur votre wallet, nous n’y avons pas accès
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Permet de prendre automatiquement tous nos trades, proportionnellement à votre capital
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Un dépôt de 200 € est requis a minima sur votre propre compte de trading
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Maximisation des intérêts composés et minimisation des pertes grâce à l’élaboration de paliers de capital
							</li>
							<li class="text-gray-300 md:text-xl leading-relaxed space-y-4">
								Les trades qui se placent sur votre compte sont ceux que nous plaçons sur notre compte
							</li>
						</lu>
				</div>
				<div class="md:shrink-0 md:max-w-md md:flex-1">
					<img
						class="h-48 rounded w-full object-cover md:h-auto md:max-w-md"
						src="concept/api-key.png"
						alt="Screenshot of a trading app "
					/>
				</div>
			</div>
		</div>
	</div>
</section>

<section id="api-key" class="w-full py-20 bg-[#181818] text-white">
	<div class="w-[90%] mx-auto text-center max-w-7xl">
		<h2 class="text-4xl text-[#F9B333] font-bold mb-6 pt-10">
			Bénéficier du service et investir en confiance
		</h2>
		<div id='paraph' class="flex flex-wrap md:flex-nowrap items-center pt-5">
			<div class="flex-1 space-y-6">
				<p class="text-gray-300 md:text-xl leading-relaxed space-y-4">
					Vous créez ou disposez d’un compte de trading Bitget.
				</p>
				<p class="text-gray-300 md:text-xl leading-relaxed space-y-4">
					Vous nous contactez pour une présentation personnelle plus détaillée.
				</p>
				<p class="text-gray-300 md:text-xl leading-relaxed space-y-4">
					Nous vous adressons un modèle de contrat.
				</p>
				<p class="text-gray-300 md:text-xl leading-relaxed space-y-4">
					Le processus s'achève par un rendez-vous physique et la signature du contrat.
				</p>
			</div>
		</div>
		<div class="text-center pt-10">
			<a href="/contact" class="inline-block  mt-6 px-6 py-3 border border-[#F9B333] bg-[#F9B333] text-black rounded-full hover:bg-black hover:text-white transition">
				Nous contacter
			</a>
		</div>
	</div>
</section>